...
var currentScore; // для отображения текущего счета
...
function prepareGameField() {
    currentScore = document.querySelector('h2');
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !haveBarrier(new_unit)) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        currentScore.innerHTML = 'Ваш счет: ' + score;
    }
    return check;
}
/**
    ...
    while (!barrierCreated) {
        ...
        if (!barrier_cell_classes.includes('snake-unit') || !barrier_cell_classes.includes('food-unit')) {
            ...
            for (var i = 0; i < barrier_cell_classes.length; i++) {
                barrierClasses += barrier_cell_classes[i] + ' ';
            }

            barrier_cell.setAttribute('class', barrierClasses + 'barrier-unit');
            barrierCreated = true;
        }
    }
}
